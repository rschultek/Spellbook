...
export function SnippetForm({ initialData, snippetId }) {
  const { register, handleSubmit, formState: { errors } } = useForm<SnippetInput>({
...
<SnippetList client:visible />  <!-- Hydrate when visible -->
<SearchBar client:idle />        <!-- Hydrate when browser idle -->
